...
<div class="card mb-4">
    <div class="card-header pb-3">
        <div class="d-flex align-items-center justify-content-between">
            <h6>From Visi Misi</h6>
        </div>
    </div>
    <div class="card-body pt-0 pb-2">
        <form action="{{ route('admin.visi.post') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <input type="number" class="form-control" name="id_visi" value="{{ $listVisi ? $listVisi->id : null }}"
                    hidden>

                <div class="form-group col-md-6">
                    <label for="">Visi</label>
                    <textarea name="visi" id="visi"
                        class="form-control">{{ $listVisi ? $listVisi->visi : null }}</textarea>
                </div>

                <div class="form-group col-md-6">
                    <label for="">Misi</label>
                    <textarea name="misi" id="misi"
                        class="form-control">{{ $listVisi ? $listVisi->misi : null }}</textarea>
                </div>
