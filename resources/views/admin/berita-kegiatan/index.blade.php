        ...
        <div class="card mb-4">
            <div class="card-header pb-3">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <a href="{{ route('admin.berita-kegiatan.form') }}" class="btn btn-sm bg-gradient-primary"><i
                            class="fas fa-plus"></i> Tambah</a>
                </div>
            </div>
            <div class="card-body pt-0 pb-2">
                <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0" id="table_berita_kegiatan">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach ($listBeritaKegiatan as $index => $item)
                            <tr>
                                ...
                                <td class="text-sm">{{ $item->tanggal_kegiatan }}</td>
                                <td class="text-sm">{{ $item->userPembuat->name }}</td>
                                <td class="text-center">
                                    <a href="{{ route('admin.berita-kegiatan.form', ['id_berita_kegiatan' => $item->id]) }}"
                                        class="btn btn-sm bg-gradient-primary" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="Edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                    <a href="{{ route('admin.berita-kegiatan.detail', ['id' => $item->id]) }}"
                                    ...
                                    <form action="{{ route('admin.berita-kegiatan.delete', ['id' => $item->id]) }}"
...
<script>
    $(document).ready(function () {
        dataTable('#table_berita_kegiatan')
        // $('#table_berita_kegiatan').DataTable()
    });
